...
export async function drawStaticMapPreview(mapData, size = 'regular', gamemode = 'Gem_Grab', environment = 'Desert', themeOptions = null, exactSize = false) {
  ...
  const needsLargePadding = gamemode === 'Brawl_Ball' || gamemode === 'Hockey' || gamemode === 'Volley_Brawl' || gamemode === 'Basket_Brawl';
  const isStuntShow = environment === 'Stunt_Show';
  // Increase padding to prevent goals/tall objects from clipping.
  const padding = (needsLargePadding || isStuntShow) ? 100 : 16;
