...
export async function drawStaticMapPreview(mapData, size = 'regular', gamemode = 'Gem_Grab', environment = 'Desert', themeOptions = null) {
    ...
    try {
        ...
        if (!sharedResources.tiles[environment][gamemode]) {
            ...
            for (const [key, img] of Object.entries(renderer.tileImages)) {
                ...
                if (isNumericKey) {
                    ...
                    if (def.showInEnvironment && !def.showInEnvironment.includes(environment))
                        continue;
                    ...
                    if (def.showInGamemode) {
                        if (Array.isArray(def.showInGamemode)) {
                            if (!def.showInGamemode.includes(gamemode))
                                continue;
                        }
                        else {
                            if (def.showInGamemode !== gamemode)
                                continue;
                        }
                    }
                }
            ...
            await Promise.all(Object.values(renderer.tileImages).filter(img => img).map(waitForImage));
        ...
        if (isBrawl) {
            ...
            await Promise.all(renderer.goalImages.map(goal => renderer.preloadGoalImage(goal.name, environment)));
        }
        ...
        // Final check - wait a tiny amount for any last-minute image loads
        // We use setTimeout instead of requestAnimationFrame to prevent complete freezing when the tab is inactive!
        ...
        for (const goal of renderer.goalImages) {
            const img = renderer.goalImageCache[`${goal.name}${environment}`] ||
            ...
            ctx.drawImage(img, goal.x * tileSize + padding + (goal.offsetX || 0) + previewOffsetX, goal.y * tileSize + padding + (goal.offsetY || 0) + previewOffsetY, (goal.w || 1) * tileSize, (goal.h || 1) * tileSize);
        }
